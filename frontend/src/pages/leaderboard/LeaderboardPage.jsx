...
export default function LeaderboardPage() {
  ...
  const getRankBadgeStyle = (rank) => {
    switch (rank) {
      case 1:
        return 'bg-amber-100 text-amber-900 border-amber-300 ring-4 ring-amber-50';
      case 2:
        return 'bg-slate-200 text-slate-800 border-slate-300 ring-4 ring-slate-100';
      case 3:
        return 'bg-orange-100 text-orange-900 border-orange-300 ring-4 ring-orange-50';
      default:
  ...
  const getMedalIcon = (rank) => {
    switch (rank) {
      case 1:
        return <Trophy className="w-5 h-5 text-amber-600 fill-amber-400" />;
      case 2:
        return <Medal className="w-5 h-5 text-slate-500 fill-slate-300" />;
      case 3:
        return <Award className="w-5 h-5 text-amber-700 fill-amber-500" />;
      default:
        return <span className="font-bold text-sm text-slate-500">#{rank}</span>;
    }
  };
  ...
  return (
    <div className="space-y-8 py-2">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 bg-white p-3.5 sm:p-4 rounded-xl border border-slate-200 shadow-xs">
        ...
        <div className="flex items-center gap-2 sm:max-w-xs w-full">
          ...
          <select
            ...
            className="w-full text-xs py-1.5 px-3 bg-slate-50 border border-slate-300 rounded-lg text-slate-800 font-medium focus:outline-hidden focus:ring-2 focus:ring-blue-600 cursor-pointer"
          >
      ...
      {isLoading ? (
        ...
        <div className="space-y-8">
          ...
          {topThree.length > 0 && (
            <div>
              <h2 className="text-xs font-bold text-slate-600 uppercase tracking-wider mb-4 flex items-center gap-1.5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {topThree.map((item) => {
                  ...
                  return (
                    <div
                      key={item._id}
                      className={`relative bg-white border rounded-2xl p-5 sm:p-6 shadow-xs flex flex-col justify-between transition-all hover:shadow-md ${
                        rank === 1
                          ? 'border-amber-300 ring-2 ring-amber-100/80 md:-translate-y-1'
                          : rank === 2
                          ? 'border-slate-300'
                          : 'border-orange-200'
                      }`}
                    >
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <div className={`w-9 h-9 rounded-xl border flex items-center justify-center font-bold shrink-0 ${rankStyle}`}>
                          {getMedalIcon(rank)}
                        </div>
                      ...
                      <div className="flex items-center gap-3.5 mb-4">
                        <div className="w-12 h-12 rounded-2xl bg-blue-700 text-white flex items-center justify-center font-bold text-base shrink-0 overflow-hidden shadow-xs">
                          {user.avatar ? (
                      ...
                      <div className="grid grid-cols-3 gap-2 pt-3 border-t border-slate-100 text-center">
                        <div className="bg-slate-50 p-2 rounded-lg border border-slate-200/60">
                          ...
                          <p className="font-bold text-xs sm:text-sm text-slate-900">{item.verifiedUploads}</p>
                        </div>

                        <div className="bg-slate-50 p-2 rounded-lg border border-slate-200/60">
                          <p className="text-[10px] text-slate-400 font-medium">Downloads</p>
                          <p className="font-bold text-xs sm:text-sm text-blue-700">{item.totalDownloads}</p>
                        </div>

                        <div className="bg-slate-50 p-2 rounded-lg border border-slate-200/60">
                          <p className="text-[10px] text-slate-400 font-medium">Avg Rating</p>
                          <p className="font-bold text-xs sm:text-sm text-amber-600 flex items-center justify-center gap-0.5">
            ...
            <div className="divide-y divide-slate-100 overflow-x-auto">
              {contributors.map((item) => {
                ...
                return (
                  <div
                    key={item._id}
                    className="p-4 sm:p-4.5 hover:bg-slate-50/80 transition-colors flex items-center justify-between gap-4 min-w-[500px]"
                  >
                    {/* Rank + User Identity */}
                    <div className="flex items-center gap-3.5 min-w-0 flex-1">
                      ...
                      <div className="w-9 h-9 rounded-xl bg-blue-700 text-white flex items-center justify-center font-bold text-xs shrink-0 overflow-hidden">
                        {user.avatar ? (
                          <img src={user.avatar} alt={user.name} className="w-full h-full object-cover" />
                        ) : (
                          user.name?.slice(0, 2).toUpperCase() || 'ST'
                    ...
                    <div className="flex items-center gap-6 sm:gap-8 text-right shrink-0">
                      <div>
                        <p className="text-[10px] text-slate-400 font-medium">Verified Notes</p>
                      ...
                      <div className="w-16">
                        ...
                        <p className="text-xs sm:text-sm font-bold text-amber-700 flex items-center justify-end gap-1">
                          <Star className="w-3 h-3 fill-amber-400 text-amber-500" />
                          <span>{item.averageRating > 0 ? item.averageRating.toFixed(1) : '5.0'}</span>
                        </p>
                      </div>
      ...
      <div className="bg-linear-to-r from-blue-700 to-indigo-800 text-white rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-sm">
        <div className="space-y-1.5 text-center sm:text-left">
          <div className="inline-flex items-center gap-1.5 bg-blue-600/60 border border-blue-400/40 px-2.5 py-0.5 rounded-full text-xs font-medium">
